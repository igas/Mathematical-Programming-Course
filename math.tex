...
\usepackage{tikz}
\usepackage{multirow}
\usetikzlibrary{arrows,calc,decorations.pathreplacing,intersections,positioning}
...
\begin{document}
  ...
  \begin{tikzpicture}[scale=.5]
  	\draw (3,0) rectangle +(4,4);
	  	\node at (5,2.5) {Модель};
  	\node at (5,1.5) {системы};
  	\foreach \y/\n in {.5/n,2.5/2,3.5/1}
  	{
  		\coordinate (l) at (0,\y);
  		\coordinate (r) at ($ (l) +(3,0)$);
  		\draw[->] (l) -- (r);
  		\node[above] at ($ (l)!.5!(r) $) {$q_{\n}$};
  	}
  	\foreach \x/\s in {3.5/1,4.5/2,6.5/s}
  	{
  		\coordinate (u) at (\x,0);
  		\coordinate (d) at ($ (u) +(0,-3) $);
  		\draw[->] (d) -- (u);
  		\node[right] at ($ (d)!.5!(u) $) {$f_{\s}$};
  	}
  	\foreach \x/\l in {3.5/1,4.5/2,6.5/l}
  	{
  		\coordinate (d) at (\x,4);
  		\coordinate (u) at ($ (d) +(0,3) $);
  		\draw[->] (u) -- (d);
  		\node[right] at ($ (u)!.5!(d) $) {$r_{\l}$};
  	}
  	\foreach \y/\m in {.5/m,2.5/2,3.5/1}
  	{
  		\coordinate (l) at (7,\y);
  		\coordinate (r) at ($ (l) +(3,0) $);
  		\draw[->] (l) -- (r);
  		\node[above] at ($ (l)!.5!(r) $) {$k_{\m}$};
  	}
  	\draw (10,0) rectangle +(4,4);
  	\node at (12,3) {Модель};
  	\node at (12,2) {принятия};
  	\node at (12,1) {решения};
  	\foreach \x/\l in {11/R,13/i}
  	{
  		\coordinate (d) at (\x,4);
  		\coordinate (u) at ($ (d) +(0,3) $);
  		\draw[->] (u) -- (d);
  		\node[right] at ($ (u)!.5!(d) $) {$\l$};
  	}
  	\draw[->] (14,2) -- (17,2);
  	\node[above] at ($ (14,2)!.5!(17,2) $) {$K$};
  	\draw (17,0) rectangle +(4.5,4);
  	\node at (19.3,3) {Модель};
  	\node at (19.3,2) {поиска};
  	\node at (19.3,1) {экстремума};
  	\foreach \y/\n in {.5/n,2.5/2,3.5/1}
  	{
  		\coordinate (l) at (21.5,\y);
  		\coordinate (r) at ($ (l) +(3,0) $);
  		\draw[->] (l) -- (r);
  		\node[above] at ($ (l)!.5!(r) $) {$q_{\n\text{о}}$};
  	}
  ...
  $q$ --- управляемые параметры\par
  $r$ --- контролируемые параметры\par
  $f$ --- возмущения\par
  $R$ --- ресурсы\par
  $i$ --- критерий оптимальности\par
  $q_{o}$ --- оптимальные параметры
  ...
  \begin{tikzpicture}
	  ...
	  \foreach \x in {0,2,3,4,8}
	  	\draw (\x,-2pt) -- (\x,2pt);
	 	\draw (2,0) node[below] {$q^{(1)}$} (3,0) node[below] {$\lambda$} (4,0) node[below] {$q^{(2)}$};
  \end{tikzpicture}
  ...
  \newline
  \begin{tikzpicture}[scale=.5]
  \node[above] at (-2.5,1) {N шага:};
  \foreach \x/\n in {1/1,2/2,4/3,8/4,16/5}
  {
  	\node[above] at (\x,1) {\n};
  }
  \draw (.5,0) node[above] {$\lambda$} (1.5,0) node[above] {$\lambda$} (12,0) node[above] {доп. точка};
  \draw (0,0) -- (16,0);
  \node[below] at (-1.5,-1) {$K$:};
  \foreach \x/\k in {0/51,1/64,2/75,4/91,8/99,12/75,16/19}
  {
  ...
  \begin{tikzpicture}
    [place/.style={rectangle,draw=black!50},sibling distance=2.7cm]

    \node[place] (x) {$X$}
    child { node[place] (x1) {$X_1$} }
    child { node[place] (x2) {$X_2$} }
    child { node[place] (x3) {$X_3$} 
      child { node[place] (x4) {$X_{31}$} }
      child { node[place] (x5) {$X_{32}$} } };
    
    \node[below] at (x1.south) {надёжность};
    \node[below] at (x2.south) {экономия};
    \node[above right] at (x3.east) {массо-габаритные};
    \node[below right] at (x3.east) {характеристики};
  \end{tikzpicture}
