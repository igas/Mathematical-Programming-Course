...
\{СХЕМА!\}
  ...
  TODO: График
  $$q_3 = q_2 + \Theta\frac{k(q_2) - k(q_1)}{\underbrace{q_2 - q_1}_{\lambda}}$$
  $$q^{(r)} = q^{(r-1)} + \Theta = \frac{k(q^{r-1}) - k(q^{r+1})}{q^{(r+1)} - q^{(r-1)}}$$
  $$\Theta = \frac{k(q^{(2)}) - k(q^{(1)})}{\lambda}$$
  ...
  \begin{tikzpicture}
    ...
    \draw[->] (-0.5,0)--(9,0) node[anchor=north] {$q$};
  	...
  	\filldraw (a2) circle (1pt) node[below] {2};
  	\filldraw (a1) circle (1pt) node[above] {1};
  	\filldraw (a4) circle (1pt) node[below] {4};
  	\filldraw (a5) circle (1pt) node[below] {5};
  	\filldraw (a6) circle (1pt) node[below] {6};
  	\filldraw (a3) circle (1pt) node[below] {3};
  	\draw[name path=curve 1] (0.5,6) .. controls ($(a2) +(0,-8)$) and ($(a1) +(0,5)$) .. (2.5,3.5);
  	...
  	\draw[name path=curve 2] (x) .. controls ($(x) +(0.3,0)$) .. (7.6,5);
  \end{tikzpicture}\\
